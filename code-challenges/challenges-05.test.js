...
const toLastNames = people => {
  let newArray = people.map(person => `${person.firstName} ${person.lastName}`)
  return newArray;
  // Solution code here...
};
...
const addValues = (arr) => {
  // Solution code here...
  let newArray = arr.reduce((acc, val, idx) => acc = acc + val, 0)
  return newArray;
};
...
const addPurchases = (arr) => {
  ...
  let newArray = arr.reduce((acc, val, idx) => acc = acc + val.purchasePrice, 0)
  return newArray;
};
...
const countNumberOfElements = (arr) => {
  ...
  let newArray = arr.reduce((acc, val, idx) => idx + 1, 0)
  return newArray;
};
